...
class AirPaintingApp:
    def __init__(self):
        ...
        # 语音识别相关
        self.voice_controller = SmartVoiceController(
            on_result_callback=self.handle_voice_command
        )

        self.voice_ui = VoiceUI(self.screen_width, self.screen_height)
    ...
    def handle_voice_command(self, text, start_time, end_time, is_final=True):
        ...
        self.voice_ui.on_voice_result(text, is_final)
        # 只有在最终结果时才进行命令处理
        if not is_final:
            return
    ...
    def run(self):
        ...
        while self.running:
            ...
            if hasattr(self, 'voice_ui'):
                self.voice_ui.update(dt)
                self.voice_ui.draw(self.screen)
